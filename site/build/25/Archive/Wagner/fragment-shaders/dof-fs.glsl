...
void main() {
	...
	sum += texture2D( tInput, ( vUv - bias * delta * blurAmount * 4. ) ) * 0.051;
	sum += texture2D( tInput, ( vUv - bias * delta * blurAmount * 3. ) ) * 0.0918;
	sum += texture2D( tInput, ( vUv - bias * delta * blurAmount * 2. ) ) * 0.12245;
	sum += texture2D( tInput, ( vUv - bias * delta * blurAmount * 1. ) ) * 0.1531;
	sum += texture2D( tInput, ( vUv + bias * delta * blurAmount * 0. ) ) * 0.1633;
	sum += texture2D( tInput, ( vUv + bias * delta * blurAmount * 1. ) ) * 0.1531;
	sum += texture2D( tInput, ( vUv + bias * delta * blurAmount * 2. ) ) * 0.12245;
	sum += texture2D( tInput, ( vUv + bias * delta * blurAmount * 3. ) ) * 0.0918;
	sum += texture2D( tInput, ( vUv + bias * delta * blurAmount * 4. ) ) * 0.051;
