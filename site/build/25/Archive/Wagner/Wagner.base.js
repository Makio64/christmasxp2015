...
WAGNER.BlendPass.prototype.run = function( c ) {
	...
	this.shader.uniforms.self.value = this.params.self;
	this.shader.uniforms.flip.value = this.params.flip;
...
WAGNER.MultiPassBloomPass.prototype.run = function( c ) {
	...
	if( this.width != this.composer.width || this.height != this.composer.height ) {
		this.composer.setSize( this.width, this.height );
	}
...
WAGNER.SSAOPass = function() {
	...
	this.params.noiseTexture = THREE.ImageUtils.loadTexture( WAGNER.assetsPath + '/noise.jpg' );
	this.params.texture = null;
	this.params.isPacked = false;
	this.params.onlyOcclusion = false;
	this.params.invProjMat = new THREE.Matrix4();
...
WAGNER.SSAOPass.prototype.run = function( c ) {
	...
	if( !this.composer ) {
		var s = 4;
		this.composer = new WAGNER.Composer( c.renderer, { useRGBA: false } );
	...
	this.shader.uniforms.invProjMat.value.copy( this.params.invProjMat );
	this.shader.uniforms.noiseTexture.value = this.params.noiseTexture;
