...
THREE.MeshLine.prototype.process = function() {
	...
	if( !this.attributes ) {
		...
		this.attributes.position.array = new Float32Array( this.positions )
		this.attributes.position.needsUpdate = true
		this.attributes.previous.array = new Float32Array( this.previous )
		this.attributes.previous.needsUpdate = true
		this.attributes.next.array = new Float32Array( this.next )
		this.attributes.next.needsUpdate = true
		this.attributes.side.array = new Float32Array( this.side )
		this.attributes.side.needsUpdate = true
		this.attributes.width.array = new Float32Array( this.width )
		this.attributes.width.needsUpdate = true
		this.attributes.uv.array = new Float32Array( this.uvs )
		this.attributes.uv.needsUpdate = true
		this.attributes.index.array = new Uint16Array( this.indices_array )
...
'        w = 1.8 * lineWidth * sin(width+time)*(1.-smoothstep(.9,1.,uv.x*1.));',
'    }',
...
'',
'    vec4 c = vColor;',
'	float alpha = 1.-smoothstep(.9,1.,vUV.x*1.);',
'		  alpha *= smoothstep(vUV.x*2.,vUV.x*2.+2.,-time/5.);',
'		  alpha *= smoothstep(hide,hide+0.15,1.-vUV.x);',
'    gl_FragColor = c*vec4(1.,1.,1.,alpha);',
'',
'}' ];
	...
	this.hide = check( parameters.hide, 0 );
