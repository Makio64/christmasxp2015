var RayMarcher = function(){
    ...
    function createNoiseTexture( name, size ){
        ...
        can.width = can.height = size || 256;
        var ctx = can.getContext( "2d" );
        var imgData = ctx.getImageData( 0,0,size, size );
        var i = imgData.data.length;
        while( i -- )imgData.data[ i ] = parseInt( Math.random() * 0xFF );
        ctx.putImageData( imgData, 0, 0 );

        var tex = new THREE.Texture(can);
        tex.needsUpdate = true;
        this.material.uniforms[ name ] = {type:'t', value:tex };
    ...
    _p.createNoiseTexture = createNoiseTexture;
